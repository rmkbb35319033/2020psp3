...
## ソースコードの説明
-InsertNewNodeについて、まずNode型のpNodeを宣言し、新しく代入するためのリスト構造体を作る。
-そして、mallocを用いてNode型の大きさだけメモリを確保する。
-その後、pNode->city,pNode->pNextに元々のリストデータを順番に代入していく。
-return値は新しく作ったリストのpNodeを返す。

-SearchCityByIDについては、Node* pNodeを宣言し、pNode->city.idと入力IDが一致するときに、resultにposを代入した。
-また、構造体pCityに見つかった市町村のアドレスを格納する。
-return値はデータの位置(pos)を代入したresultを返す。
